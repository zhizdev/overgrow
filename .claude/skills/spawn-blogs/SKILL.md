...
user-invocable: true
argument-hint: "[optional: pillar name or topic seed]"
license: MIT
...
3. Parse `$ARGUMENTS`: if a pillar name or topic seed is given, scope to it. If not, propose up to 3 candidate pillars to expand and STOP and call the AskUserQuestion tool to clarify. which to run. Once chosen, proceed for that pillar.
...
- Follow the project's voice if a brand voice doc exists (check `CLAUDE.md`, `BRAND.md`, `VOICE.md`, `style-guide.md`). Otherwise default to clear, active, jargon-light.
