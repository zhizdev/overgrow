...
user-invocable: true
argument-hint: "[audit | build] [optional: site origin]"
license: MIT
...
2. Parse `$ARGUMENTS`:
   ...
   - No args: if an `llms.txt` already exists in the repo, default to audit; otherwise default to build. If the project's intent is unclear, STOP and call the AskUserQuestion tool to clarify.
