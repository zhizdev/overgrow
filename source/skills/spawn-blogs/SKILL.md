...
3. Parse `$ARGUMENTS`: if a pillar name or topic seed is given, scope to it. If not, propose up to 3 candidate pillars to expand and {{ask_instruction}} which to run. Once chosen, proceed for that pillar.
...
- Follow the project's voice if a brand voice doc exists (check `{{config_file}}`, `BRAND.md`, `VOICE.md`, `style-guide.md`). Otherwise default to clear, active, jargon-light.
